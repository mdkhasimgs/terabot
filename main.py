...
ADMIN_ID = 7598595878
FORCE_JOIN_CHANNEL = "@hahahanakak"
UPLOAD_CHANNEL = "@hahahanakak"    # used if you also post to a channel
...
async def force_join_keyboard():
    return ReplyKeyboardMarkup([["Join Channel"]], resize_keyboard=True, one_time_keyboard=True)

async def check_force_join(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    try:
        chat_member = await context.bot.get_chat_member(FORCE_JOIN_CHANNEL, update.effective_user.id)
...
async def create_verification_links(context: ContextTypes.DEFAULT_TYPE, user_id: str) -> dict:
    """Create a single-use verification token and return both shortener links"""
    token = generate_token(12)
    expires_at = datetime.now(timezone.utc) + timedelta(minutes=SHRINK_VERIFY_EXPIRY_MIN)
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🎥 Set More Videos Link", callback_data="set_more_videos")],
...
async def admin_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "broadcast":
        ...
        await query.edit_message_text("Send the media file (photo, video, or document):")
        return UPLOAD_FILE
        ...
        await query.edit_message_text("Send the new channel link for 'More Videos':")
        ...
        await query.edit_message_text("Send up to 10 media files (photo/video/document). Type /done when finished.")
...
async def add_get_userid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ User ID set: {user_id}\nNow send how many views to add:")
    return ADD_VIEWS
...
async def add_get_views(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_id:
        await update.message.reply_text("⚠️ User ID missing. Restart with /add.")
        return ConversationHandler.END
...
async def store_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await check_force_join(update, context):
        return ConversationHandler.END
    await update.message.reply_text("Send the media file (photo, video, or document) for your post:")
    return UPLOAD_FILE
...
async def receive_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        await update.message.reply_text("Unsupported media type! Send photo, video, or document only.")
        return UPLOAD_FILE
    ...
    await update.message.reply_text("✅ File received. Now send the title for this post.")
    return UPLOAD_TITLE
    ...
    try:
        if f["media_type"] == "photo":
            await update.get_bot().send_photo(
                chat_id=UPLOAD_CHANNEL, photo=f["media_id"], caption=title, protect_content=True
            )
        elif f["media_type"] == "video":
            await update.get_bot().send_video(
                chat_id=UPLOAD_CHANNEL, video=f["media_id"], caption=title, supports_streaming=True, protect_content=True
            )
        elif f["media_type"] == "document":
            await update.get_bot().send_document(
                chat_id=UPLOAD_CHANNEL, document=f["media_id"], caption=title, protect_content=True
            )
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith("verify_"):
        ...
        if not tok_doc.exists:
            ...
            tok = tok_doc.to_dict()
            if tok.get("used"):
                await update.message.reply_text("⚠️ This verification link was already used.")
            elif tok.get("user_id") != user_id:
                await update.message.reply_text("❌ This verification link does not belong to you.")
            else:
                try:
                ...
                if datetime.now(timezone.utc) > exp:
                    await update.message.reply_text("⌛ Verification link expired. Request a new one.")
                else:
                    # Mark used and extend allowance
                    tok_ref.update({"used": True, "used_at": datetime.now(timezone.utc).isoformat()})
                    user_doc = db.collection("users").document(user_id)
                    ud = user_doc.get().to_dict() or {}
                    views_used = int(ud.get("views_used", 0))
                    user_doc.set({"verified_until": views_used + 10}, merge=True)
                    await update.message.reply_text("✅ Verification successful! You unlocked 10 more posts.")
        # continue to menu
    ...
    if context.args and not context.args[0].startswith("verify_"):
        ...
        if not post.exists:
            ...
            if views_used >= verified_until:
                ...
                try:
                    if f["media_type"] == "photo":
                        await update.message.reply_photo(f["media_id"], protect_content=True)
                    elif f["media_type"] == "video":
                        await update.message.reply_video(
                            f["media_id"], supports_streaming=True, protect_content=True
                        )
                    elif f["media_type"] == "document":
                        await update.message.reply_document(f["media_id"], protect_content=True)
                except Exception as e:
                    logger.error(f"Failed to send media: {e}")
                    await update.message.reply_text("Failed to send media. It might be too large or unavailable.")
...
async def user_storage(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def send_card_for_post(post_id: str, d: dict):
        ...
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✏ Edit Title", callback_data=f"edit_{post_id}")],
            [InlineKeyboardButton("🗑 Delete", callback_data=f"delete_{post_id}")],
            [InlineKeyboardButton("♻ Replace", callback_data=f"replace_{post_id}")],
        ])
        if d["file"]["media_type"] == "video" and d["file"]["file_info"].get("thumb_id"):
            thumb_id = d["file"]["file_info"]["thumb_id"]
            await update.message.reply_photo(
                photo=thumb_id,
                caption=post_card_caption(d["title"], d.get("views", 0), link),
                reply_markup=kb,
                protect_content=True
            )
        else:
            await update.message.reply_text(
                text=post_card_caption(d["title"], d.get("views", 0), link),
                reply_markup=kb
            )
...
async def more_videos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    link = cfg.get("more_videos", "https://t.me/example_channel")
...
async def handle_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for u in db.collection("users").stream():
        try:
            await context.bot.send_message(int(u.id), text, protect_content=True)
            count += 1
        except Exception:
            continue
...
async def set_more_videos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.collection("config").document("settings").set({"more_videos": link}, merge=True)
    await update.message.reply_text(f"✅ 'More Videos' link updated to:\n{link}")
    context.user_data.pop("awaiting_more_videos_link", None)
    return ConversationHandler.END

# ---------------- MULTI UPLOAD (ADMIN) ----------------
async def multi_upload_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.user_data["multi_posts"]) >= 10:
        await update.message.reply_text("❌ Max 10 posts allowed. Type /done now.")
        return MULTI_UPLOAD_FILE
    ...
    if update.message.photo:
        ...
        await update.message.reply_text("Unsupported type! Send photo, video, or document.")
        return MULTI_UPLOAD_FILE
    ...
    await update.message.reply_text(f"✅ File {len(context.user_data['multi_posts'])}/10 received. Send next or /done.")
    return MULTI_UPLOAD_FILE

async def multi_upload_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in posts:
        ...
        try:
            f = p["file"]
            if f["media_type"] == "photo":
                await update.get_bot().send_photo(UPLOAD_CHANNEL, f["media_id"], caption=p["title"], protect_content=True)
            elif f["media_type"] == "video":
                await update.get_bot().send_video(UPLOAD_CHANNEL, f["media_id"], caption=p["title"], supports_streaming=True, protect_content=True)
            elif f["media_type"] == "document":
                await update.get_bot().send_document(UPLOAD_CHANNEL, f["media_id"], caption=p["title"], protect_content=True)
        except Exception as e:
...
def run_bot():
    ...
    conv = ConversationHandler(
        ...
        states={
            UPLOAD_FILE: [MessageHandler(filters.ALL & ~filters.COMMAND, receive_file)],
            UPLOAD_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_title)],
            SET_MORE_VIDEOS: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_more_videos)],
            EDIT_TITLE: [MessageHandler(filters.TEXT & ~filters.COMMAND, edit_title)],
            REPLACE_FILE: [MessageHandler(filters.ALL & ~filters.COMMAND, replace_file)],

            # /done handled inside state
            MULTI_UPLOAD_FILE: [
                MessageHandler(filters.ALL & ~filters.COMMAND, multi_upload_file),
                CommandHandler("done", multi_upload_done),
            ],

            # /add states
            ADD_USER_ID: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_get_userid)],
            ADD_VIEWS: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_get_views)],
        },
    ...
    application.add_handler(CallbackQueryHandler(
        admin_button,
        pattern="^(broadcast|upload|total_users|set_more_videos|multi_post|uptime)$"
    ))

    # Storage callbacks
    application.add_handler(CallbackQueryHandler(storage_button,
    ...
    application.add_handler(MessageHandler(filters.Regex("^🗂 Storage$"), user_storage))
    application.add_handler(MessageHandler(filters.Regex("^🎥 More Videos$"), more_videos))
...
if __name__ == "__main__":
    try:
        asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())  # harmless elsewhere
    except Exception:
        pass
    start_server()
